...
var appendFrame = function(o) {
	...
	if(o.buttons) {
		$.each(o.buttons, function(i, button) {
			additionnalButtons += '<input type="button" id="sti_' + o.id + '_' + button.b_id + '" class="fine" value=" ' + button.b_text + ' "> ';
		});
	}

	// Build entire frame
	var frame = $("<div>", {id: "sti_" + o.id, class: "sti_frame" + (o.classes ? ' ' + o.classes : '')}).append(
		$("<p>", {class: "separate", text: o.title}),
		$("<div>", {class: "sti_frame_content"}).append(
			(o.header ? $("<div>", {class: "sti_frame_header"}).append(o.header) : $()),
			$("<div>", {id: "sti_" + o.id + "_data", class: "sti_frame_data"}).append(o.data),
			$("<div>", {id: "sti_" + o.id + "_buttons", class: "sti_frame_buttons"}).append(
	...
	// Append
	$("#navigation").append(frame);

	// Close button
	frame.find("#sti_" + o.id + "_close").click(function() {
	...
	if(additionnalButtons.length) {
		$.each(o.buttons, function(i, button) {
			if(button.b_callback) {
				frame.find("#sti_" + o.id + "_" + button.b_id).click(function() {
					button.b_callback(frame.find("#sti_" + o.id + "_data"), "#sti_" + o.id);
					return false;
				});
			}
		});
	...
	if(frame.find(".sti_frame_content").css("background-color") == transparentCss || frame.find(".sti_frame_data").css("background-color") == transparentCssFirefox) {
		...
		$.each(cssTries, function(i, cssId) {
			...
			if(cssColor != transparentCss && cssColor != transparentCssFirefox) {
				...
				var colorRGBA = cssColor.match(/^rgba\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*([\.\d]+)\s*\)$/i);
				if(colorRGBA) {
				...
				appendCSS('.sti_frame_content { background-color: ' + cssColor + '; } ');
				return false;
			}
		});
	}
...
var insertCSS = function() {
	...
	$("head").append(["<style id='sti_css'>",
		...
		// Frames
		".sti_frame { z-index: 10; position: absolute; }",
		".sti_frame_content { width: auto; padding: 12px; }",
		".sti_frame_header { padding-top: 6px; padding-bottom: 6px; }",
		//".sti_frame_data { }",
		".sti_frame_section_header { border-bottom: 1px solid; font-weight: bold; padding-top: 6px; }",
		".sti_frame_buttons { padding-top: 9px; text-align: center; } ",
		//"#sti_suggest { }",
		//"#sti_suggest_data { }",
		//"#sti_options { }",
		"#sti_options_data { min-height: 96px; }",
		".sti_options_header_button { display: inline-block; background-color:#f5f5f5; border:1px solid #dedede; border-top:1px solid #eee; border-left:1px solid #eee; font-weight:bold; color:#565656; cursor:pointer; padding:5px 10px 6px 7px; }",
		".sti_options_header_button_selected { background-color:#6299c5; color:#fff; }",
		".sti_options_sub { font-size: 0.9em; padding-left: 12px; }",
		".sti_options_sub input { margin:2px; }",
		".sti_option_required { text-decoration: underline; }",
		".sti_opt_has_parent { margin-left: 12px; }",
		"#sti_copyright { text-align: right; font-size: 0.8em; margin-top: -11px; }",
		".sti_edit_title { display: block; width: 100%; margin-top: 12px; }",
		".sti_edit_textarea { display: block; width: 100%; min-height: 240px; min-width: 360px; margin-right: -4px!important; }",
		".sti_preview { position: absolute; z-index: 160; }",
		".sti_preview img { max-width: 300px; }",
		"#sti_marker { max-width: 310px; }",
		...
		// Badges
		".sti_progress_area { margin-top: 4px; display: inline-block; width: 90px; border-radius: 2px; padding: 1px; border: 1px solid gray; font-size: 9px; }",
		".sti_progress_bar { background-color: orange; height: 11px; border-radius: 1px; margin-bottom: -11px; }",
		".sti_progress_numbers { position: relative; }",
		".sti_valid { background-color: lightgreen; }",
		".sti_selected { background-color: aquamarine; }",

		// Torrent list columns
		".age_torrent_head { width: 40px; text-align: center; font-weight: bold; background-color: #D3D3D3; }",
		".age_torrent_0 { width: 40px; text-align: center; }",
		".age_torrent_1 { width: 40px; text-align: center; background-color: #f6f6f6; }",
		".autoget_torrent_head { width: 26px; text-align: center; font-weight: bold; background-color: #D3D3D3; }",
		".autoget_torrent_0 { width: 26px; text-align: center; }",
		".autoget_torrent_1 { width: 26px; text-align: center; background-color: #f6f6f6; }",
		".bookmark_torrent_head { width: 30px; text-align: center; font-weight: bold; background-color: #D3D3D3; }",
		".bookmark_torrent_0 { width: 30px; text-align: center; }",
		".bookmark_torrent_1 { width: 30px; text-align: center; background-color: #f6f6f6; }",
		".nfo_torrent_head { width: 30px; text-align: center; font-weight: bold; background-color: #D3D3D3; }",
		...
		// Aura
		"#sti_aura_controls thead th { width: 33%; }",
		"#sti_aura_controls tbody td { vertical-align: inherit; }",
		".sti_aura_result { font-weight: bold; }",
		...
		".sig_separator { cursor: pointer; }",
		".sig_separator:hover { text-decoration: underline; }",
		"</style>"].join(" "));
};
